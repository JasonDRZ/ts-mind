...
import { doc, isElement, eleByID } from "utils/dom";
...
interface IMViewProvOptions {
  [k: string]: any;
}
type IMViewEle<T = HTMLElement> = T | null;

export default class VNodeView {
  static addr = "vnv";
  private mind: TSMind;
  // public conf
  public opts: IMViewProvOptions;
  public container: HTMLElement;
  public e_panel: HTMLElement = doc.createElement("div");
  public e_nodes: HTMLElement = doc.createElement("div");
  public e_canvas: HTMLCanvasElement = doc.createElement("canvas");
  public e_editor: HTMLInputElement = doc.createElement("input");
  public canvas_ctx: CanvasRenderingContext2D = this.e_canvas.getContext("2d") as CanvasRenderingContext2D;
  public size = { w: 0, h: 0 };
  public selected_node: IMViewEle<VNode> = null;
  public editing_node: IMViewEle<VNode> = null;
  // view zoom
  private actualZoom = 1;
  private zoomStep = 0.1;
  private minZoom = 0.5;
  private maxZoom = 2;
  constructor(mind: TSMind, options: IMViewProvOptions) {
    this.opts = options;
    this.mind = mind;
    this.container = isElement(this.opts.container) ? this.opts.container : eleByID(this.opts.container);
    if (!this.container) {
      this.mind.logger.error("the options.view.container was not be found in dom");
      return;
    }
  ...
  // add_event = (obj: {}, event_name: IMEventType, event_handle: IMAnyCall) => {
  //   this.e_nodes &&
  ...
  reset = () => {
    this.mind.logger.debug("view.reset");
  ...
  reset_theme = () => {
    const theme_name = this.mind.options.theme;
    if (!!theme_name) {
  ...
  reset_custom_style = () => {
    const nodes = this.mind.mind!.nodes;
  ...
  load = () => {
    this.mind.logger.debug("view.load");
    this.init_nodes();
  };
  ...
  create_node_element = (node: VNode, parent_node: HTMLElement | DocumentFragment) => {
  ...
  select_node = (node: null | VNode) => {
    ...
    if (!!selected_node_ele) {
      ...
      this.reset_node_custom_style(this.selected_node as VNode);
    }
    if (!!node && !!node_ele) {
  ...
  edit_node_begin = (node: VNode) => {
    if (!node.topic) {
      this.mind.logger.warn("don't edit image nodes");
      return;
    }
  ...
  edit_node_end = () => {
    if (this.editing_node != null) {
      ...
      if (!topic || node.topic === topic) {
        ...
        this.mind.update_node(node.id, topic);
      }
    }
  };
  ...
  _show = () => {
    ...
    this.mind.invoke_event_handle(TSMindEventTypeMap.resize, { data: [] });
  };
  ...
  show = (keep_center: boolean) => {
    this.mind.logger.debug("view.show");
    this.expand_size();
    this._show();
    if (keep_center) {
      this._center_root();
  ...
  reset_node_custom_style = (node: VNode) => {
    this._reset_node_custom_style(node.view_data.element as HTMLElement, node.data);
  };
  ...
  clear_node_custom_style = (node: VNode) => {
    const node_element = node.view_data.element;
    if (node_element) {
  ...
  clear_lines = (canvas_ctx: CanvasRenderingContext2D = this.canvas_ctx as CanvasRenderingContext2D) => {
    canvas.clear(canvas_ctx, 0, 0, this.size.w, this.size.h);
  };

  show_lines = (canvas_ctx: CanvasRenderingContext2D = this.canvas_ctx as CanvasRenderingContext2D) => {
  ...
  draw_line = (
    ...
    pout: IM2DAxisData,
    offset: IM2DAxisData,
    canvas_ctx: CanvasRenderingContext2D = this.canvas_ctx as CanvasRenderingContext2D
  ) => {
