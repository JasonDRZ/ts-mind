...
export const node_array = {
  example: {
    ...
    format: "node_array",
    data: [{ id: "root", topic: "jsMind Example", isRoot: true }] as ITSMNodeDataItem[]
  },
  ...
  _parse(mind: TSM_mind, nodes: any) {
    const df = node_array;
  ...
  _extract_root(mind: TSM_mind, nodes: any) {
    const df = node_array;
    let i = nodes.length;
    while (i--) {
  ...
  _extract_subnode(mind: TSM_mind, parentid: string, nodes: any) {
    const df = node_array;
    let i = nodes.length;
    let node_json = null;
    let data = null;
    ...
    while (i--) {
      ...
      if (node_json.parentid === parentid) {
        ...
        mind.add_node(
          ...
          data,
          undefined,
          node_direction ? (node_direction === "left" ? TSMind.direction.left : TSMind.direction.right) : undefined,
          node_json.expanded
        );
        nodes.splice(i, 1);
        extract_count++;
  ...
  _array(mind: TSM_mind, nodes: any) {
    const df = node_array;
    mind.root && df._array_node(mind.root, nodes);
  },

  _array_node(node: TSM_node, nodes: any) {
    const df = node_array;
    if (!(node instanceof TSM_node)) {
      return;
    }
    const o: ITSMNodeDataItem = {
      id: node.id,
      topic: node.topic,
