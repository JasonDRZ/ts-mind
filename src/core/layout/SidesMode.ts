...
export class SidesMode {
  ...
  layout(vm: Mind, force: boolean = false): IMLayoutMindBorder {
    const { leftTop: l_tl, rightBottom: l_rb } = vm.layout.getModeLayout(LayoutModeEnum.left).layout(vm, true);
    const { leftTop: r_tl, rightBottom: r_rb } = vm.layout.getModeLayout(LayoutModeEnum.right).layout(vm, true);
    return {
